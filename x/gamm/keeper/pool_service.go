...
) (spotPrice math.LegacyDec, err error) {
...
// each pool asset.
func getMaximalNoSwapLPAmount(ctx sdk.Context, pool types.CFMMPoolI, shareOutAmount math.Int) (neededLpLiquidity sdk.Coins, err error) {
	totalSharesAmount := pool.GetTotalShares()
	// shareRatio is the desired number of shares, divided by the total number of
	// shares currently in the pool. It is intended to be used in scenarios where you want
	shareRatio := math.LegacyNewDecFromInt(shareOutAmount).QuoInt(totalSharesAmount)
	if shareRatio.LTE(math.LegacyZeroDec()) {
	...
	for _, coin := range poolLiquidity {
		...
		neededAmt := math.LegacyNewDecFromInt(coin.Amount).Mul(shareRatio).Ceil().RoundInt()
		if neededAmt.LTE(math.ZeroInt()) {
			return sdk.Coins{}, sdkerrors.Wrapf(types.ErrInvalidMathApprox, "Too few shares out wanted")
		}
...
func (k Keeper) JoinSwapExactAmountIn(
	...
	case sharesOut.LTE(math.ZeroInt()):
		return math.ZeroInt(), sdkerrors.Wrapf(types.ErrInvalidMathApprox, "share amount is zero or negative")
	}
...
func (k Keeper) JoinSwapShareAmountOut(
	...
	if !ok {
		return math.Int{}, fmt.Errorf("pool with id %d does not support this kind of join", poolId)
	}
	...
	if tokenInAmount.GT(tokenInMaxAmount) {
		return math.Int{}, sdkerrors.Wrapf(types.ErrLimitMaxAmount, "%s resulted tokens is larger than the max amount of %s", tokenInAmount, tokenInMaxAmount)
	}
...
func (k Keeper) ExitSwapShareAmountIn(
	...
	for _, coin := range exitCoins {
		...
		swapOut, err := k.SwapExactAmountIn(ctx, sender, pool, coin, tokenOutDenom, math.ZeroInt(), swapFee)
		if err != nil {
			return math.Int{}, err
		}
	...
	if tokenOutAmount.LT(tokenOutMinAmount) {
		return math.Int{}, sdkerrors.Wrapf(types.ErrLimitMinAmount,
...
func (k Keeper) ExitSwapExactAmountOut(
	...
	extendedPool, ok := pool.(types.PoolAmountOutExtension)
	if !ok {
		return math.Int{}, fmt.Errorf("pool with id %d does not support this kind of exit", poolId)
	}
