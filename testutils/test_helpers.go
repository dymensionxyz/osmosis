...
func Setup(isCheckTx bool, chainID string) *dymapp.App {
	db := dbm.NewMemDB()
	encCdc := dymapp.MakeEncodingConfig()
	if chainID == "" {
		chainID = TestChainID
	}
	app := dymapp.New(log.NewNopLogger(), db, nil, true, map[int64]bool{}, dymapp.DefaultNodeHome, 0, encCdc, simapp.EmptyAppOptions{}, baseapp.SetChainID(chainID))

	if !isCheckTx {
		genesisState := dymapp.NewDefaultGenesisState(encCdc.Codec)
...
func SetupTestingAppWithLevelDb(isCheckTx bool) (app *dymapp.App, cleanupFn func()) {
	...
	encCdc := dymapp.MakeEncodingConfig()
	app = dymapp.New(log.NewNopLogger(), db, nil, true, map[int64]bool{}, dymapp.DefaultNodeHome, 5, encCdc, simapp.EmptyAppOptions{}, baseapp.SetChainID(TestChainID))
...
func genesisStateWithValSet(
	app *dymapp.App, genesisState dymapp.GenesisState,
	valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount,
	balances ...banktypes.Balance,
) (dymapp.GenesisState, error) {
