...
var smallestDec = math.LegacySmallestDec()
...
func MonotonicSqrt(d math.LegacyDec) (math.LegacyDec, error) {
	if d.IsNegative() {
	...
	root := math.LegacyNewDecFromBigIntWithPrec(r, 18)

	return root, nil
}

// MustMonotonicSqrt returns the output of MonotonicSqrt, panicking on error.
func MustMonotonicSqrt(d math.LegacyDec) math.LegacyDec {
