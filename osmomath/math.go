...
var (
	one_half math.LegacyDec = math.LegacyMustNewDecFromStr("0.5")
	one      math.LegacyDec = math.LegacyOneDec()
	two      math.LegacyDec = math.LegacyMustNewDecFromStr("2")
...
// for more detail.
func GetPowPrecision() math.LegacyDec {
...
// func largeBasePow(base math.LegacyDec, exp math.LegacyDec) math.LegacyDec {
...
// Contract: 0 < base <= 2
// 0 <= exp < 1.
func PowApprox(base math.LegacyDec, exp math.LegacyDec, precision math.LegacyDec) math.LegacyDec {
	if !base.IsPositive() {
		panic(fmt.Errorf("base must be greater than 0"))
	}
	...
	for i := int64(1); term.GTE(precision); i++ {
		...
		// On this line, bigK == i.
		bigK.Set(math.LegacyNewDec(i)) // TODO: O(n) bigint allocation happens
